...
function HighlightMark({
  ...
  return (
    <mark
      ...
      style={{
        backgroundColor: highlightColors[mark] ?? highlightColors.highlight,
      }}
    >
      {children}
    </mark>
...
const components: Partial<PortableTextReactComponents> = {
  block: {
    ...
    normal: ({ children }) => (
      <p className="mt-5 text-pretty text-muted">{children}</p>
    ),
    blockquote: ({ children }) => (
  ...
  marks: {
    ...
    highlightYellow: ({ children }) => (
      <HighlightMark mark="highlightYellow">{children}</HighlightMark>
    ),
    highlightGreen: ({ children }) => (
      <HighlightMark mark="highlightGreen">{children}</HighlightMark>
    ),
    highlightCyan: ({ children }) => (
      <HighlightMark mark="highlightCyan">{children}</HighlightMark>
    ),
    highlightRed: ({ children }) => (
      <HighlightMark mark="highlightRed">{children}</HighlightMark>
    ),
    highlightPurple: ({ children }) => (
      <HighlightMark mark="highlightPurple">{children}</HighlightMark>
    ),
    textColor: ({ children, value }) => {
      const color =
        textColors[(value as ColorMarkValue | undefined)?.color ?? "default"];
    ...
    backgroundColor: ({ children, value }) => {
      const backgroundColor =
        backgroundColors[
          (value as ColorMarkValue | undefined)?.color ?? "cyan"
        ];

      return (
        <span
          className="rounded-none px-1 text-ink"
  ...
  types: {
    ...
    callout: ({ value }) => (
      <aside className="stamp-panel my-8 p-5 text-ink">
        <p className="font-tech text-xs font-bold text-primary-strong">
          {value.tone ?? "Insight"}
        </p>
          ...
          {items.map((item, index) => (
            <li key={`${item.text}-${index}`} className="flex gap-3">
              ...
              <span
                className={
                  item.checked
                    ? "text-muted line-through decoration-[#00d2ff]/70"
                    : undefined
                }
              >
                {item.text}
        ...
        <figure className="my-8 overflow-hidden rounded-none border border-[#00d2ff]/35 bg-terminal">
          {codeBlock.filename || codeBlock.language ? (
            <figcaption className="flex items-center justify-between gap-4 border-b border-[#00d2ff]/25 px-4 py-2 font-tech text-xs font-bold text-muted">
              ...
              {codeBlock.language ? (
                <span className="shrink-0 uppercase text-primary-strong">
                  {codeBlock.language}
                </span>
              ) : null}
            </figcaption>
    ...
    webBookmark: ({ value }) => {
      ...
      return (
        ...
        >
          ...
          <span className="mt-4 block truncate font-tech text-xs font-bold text-muted">
            {bookmark.url}
          </span>
        </a>
      );
    ...
    mermaidDiagram: ({ value }) => {
      ...
      return (
        <figure className="my-8 overflow-hidden rounded-none border border-[#00d2ff]/35 bg-terminal">
          ...
          {diagram.caption ? (
            <figcaption className="border-t border-[#00d2ff]/25 px-4 py-3 text-sm text-muted">
              {diagram.caption}
            </figcaption>
          ) : null}
        </figure>
            ...
            <thead className="bg-terminal text-ink">
              <tr>
                {head?.map((cell, index) => (
                  <th
                    key={`${cell}-${index}`}
                    className="border border-[#00d2ff]/35 px-4 py-3 font-tech font-bold"
                  >
                    {cell}
                  </th>
                ))}
              ...
              {body.map((row, rowIndex) => (
                <tr key={rowIndex} className="odd:bg-surface-strong/40">
                  {row.cells?.map((cell, cellIndex) => (
                    <td
                      key={`${cell}-${cellIndex}`}
                      className="border border-[#00d2ff]/25 px-4 py-3 align-top"
                    >
                      {cell}
                    </td>
                  ))}
  ...
  list: {
    bullet: ({ children }) => (
      <ul className="mt-5 list-disc space-y-2 pl-6 text-muted">{children}</ul>
    ),
    number: ({ children }) => (
      <ol className="mt-5 list-decimal space-y-2 pl-6 text-muted">
        {children}
      </ol>
    ),
    checkmarks: ({ children }) => (
      <ul className="mt-5 list-none space-y-2 pl-0 text-muted">{children}</ul>
    ),
  },
  listItem: {
    checkmarks: ({ children }) => (
      <li className="flex gap-3">
        <span
          className="mt-0.5 grid size-5 shrink-0 place-items-center border border-[#00d2ff]/45 font-tech text-[0.68rem] font-bold text-primary-strong"
          aria-hidden="true"
        >
          x
        </span>
      ...
      {blocks.map((block, index) => {
        ...
        if (trimmed.startsWith("### ")) {
          return (
            <h3
              key={key}
              className="mt-9 font-display text-2xl font-black leading-[1.35] text-balance text-ink"
            >
        ...
        if (trimmed.startsWith("## ")) {
          return (
            <h2
              key={key}
              className="mt-12 font-display text-3xl font-black leading-[1.35] text-balance text-ink"
            >
        ...
        if (/^\d+\.\s+/m.test(trimmed)) {
          return (
            <ol
              key={key}
              className="mt-5 list-decimal space-y-2 pl-6 text-muted"
            >
              {trimmed.split("\n").map((item) => (
                <li key={item}>
...
export function RichTextRenderer({ value }: RichTextRendererProps) {
  ...
  return (
    <div className="prose-content">
      <PortableText
        value={addMarkdownHighlightMarks(value)}
